import { Forecast } from '@smarthealth/types';
...
const generateVillageForecast = (villageId: string, baseCases: number, riskPattern: ('GREEN' | 'AMBER' | 'RED')[]): Forecast[] => {
  return riskPattern.map((risk, index) => {
    let predicted = baseCases;
    if (risk === 'AMBER') predicted += Math.floor(Math.random() * 10) + 5;
    if (risk === 'RED') predicted += Math.floor(Math.random() * 20) + 15;
    if (risk === 'GREEN') predicted = Math.max(0, predicted - Math.floor(Math.random() * 5));
